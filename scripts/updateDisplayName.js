...
async function updateDisplayNames() {
  try {
    ...
    users.forEach((user, index) => {
      console.log(`${index + 1}. 使用者名稱: ${user.username}`);
    ...
    const newDisplayName = await question('請輸入新的顯示名稱: ');

    if (!newDisplayName.trim()) {
      console.log('❌ 顯示名稱不能為空!');
      process.exit(1);
    }
    ...
    console.log(`\n✅ 已更新 ${selectedUser.username} 的顯示名稱為: ${newDisplayName.trim()}`);
